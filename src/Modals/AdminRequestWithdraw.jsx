...
const AdminRequestWithdraw = () => {
  ...
  const [withdrawalIdValue, setWithdrawalIdValue] = useState("");
  ...
  const [showWithdrawalId, setShowWithdrawalId] = useState(false);

  const handleSelectCurrent = (item) => {
    setShowWithdrawalId(true);
  ...
  const createWithdraw = async () => {
    ...
    const payload = {
      ...
      user_type: "affilate",
      payable: "",
      deposit_amount: inputAmount,
      bonus: selectedGateway?.bonus,
      after_deposit: Number(adminData?.amount) + Number(inputAmount),
  ...
  const handleCryptoWithdraw = async () => {
    ...
    if (!inputAmount) {
      toast({
        title: "Please enter a valid amount to continue.",
        status: "error",
        duration: 2000,
        position: "top",
        isClosable: true,
    ...
    if (!selectedCurrency) {
      toast({
        title: "Please select a payment gateway to proceed.",
        status: "error",
        duration: 2000,
        position: "top",
        isClosable: true,
    ...
    if (!withdrawalIdValue) {
      toast({
        title: "Please enter a valid address to proceed.",
        status: "error",
        duration: 2000,
        position: "top",
        isClosable: true,
    ...
    try {
      const payload = {
        ...
        payment_id: selectedCurrency?.id,
        address_to: withdrawalIdValue,
      };

      const response = await sendPostRequest(
        `${
          import.meta.env.VITE_API_URL
        }/api/auto-payment/withdraw-passim-pay-funds`,
        payload
      );

      const data = response.data;
      toast({
        description: "Please enter a valid address to proceed.",
        title: "",
        status: "success",
        duration: 2000,
        position: "top",
      ...
      setNextLoader(false); // Reset loader state
    }
  };
  ...
  return (
    <>
      ...
      >
        <RiLuggageDepositFill fontSize={"20px"} /> {t(`Withdraw`)}{" "}
        {t(`Request`)}
      </button>
      <Modal
      ...
      >
        ...
        <ModalContent>
          <ModalHeader className="text-center">Withdraw Request</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Text className="font-semibold text-sm">Enter Withdraw Amount</Text>
            <Input
              value={inputAmount}
            ...
            />
            {selectedGateway && (
              <div className="mb-2 flex mt-1 justify-between text-xs font-bold">
                <p>Min/Max</p>
                <p>
                  {selectedGateway?.min_limit}/{selectedGateway?.max_limit}
                </p>
              </div>
            )}
                ...
                {cryptoLoader ? (
                  ...
                  <>
                    ...
                    {showWithdrawalId && (
                      <div>
                        <input
                          onChange={(e) => setWithdrawalIdValue(e.target.value)}
                          placeholder="Enter withdrawal address (e.g., 0x123.... or 1A1z...)"
                          style={{ border: `1px solid ${border}60` }}
                          className="p-2 w-full rounded-md mt-2 "
                        />
                      </div>
                    )}
                  </>
...
export default AdminRequestWithdraw;
