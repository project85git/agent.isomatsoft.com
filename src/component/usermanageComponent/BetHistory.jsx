  ...
  return (
    ...
    <div onClick={()=>{
      ...
      <p className="text-lg font-bold text-green-600">{betsCount?.pendingBet}</p>
        ...
        <div className='overflow-auto bg-white mt-5 p-4 rounded-md'>
        {loading && (
              <Progress size="xs" isIndeterminate colorScheme="#e91e63" />
            )}
        <table className={`w-full`}>
      <thead>
        <tr style={{ borderBottom: `1px solid ${border}60` }} className={`text-center p-2 rounded-md h-[30px] text-[10px] font-bold`}>
          <th className="text-left min-w-[150px] ">{t(`Username`)} / {t(`User`)} {t(`id`)}</th>
          <th className='min-w-[120px] '>{t(`Game`)} {t(`id`)} </th>
      ...
      <tbody>
        {casinoBetData &&
          casinoBetData?.map((item) => {
            return (
              <tr key={item?._id} style={{ borderBottom: `1px solid ${border}60` }} className={`text-left h-[60px] m-auto text-xs`}>
                <td className="">
                ...
                <td className=''>
                  ...
                  <p> {formatDateTime(item.BetTime).split(" ")[1]}</p>
                ...
                <td className={`font-bold  `}><Badge> {item.Status}</Badge></td>
