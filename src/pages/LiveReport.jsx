...
import { MdLiveTv, MdWorkHistory } from "react-icons/md";
...
const LiveReport = () => {
  ...
  const getCasinoBetHistory = async () => {
    ...
    }/api/bet/get-all-bet?status=pending&page=${currentPage}&limit=${limit}`;
  ...
  <Stack spacing={4} wrap={"wrap"}  direction='row'>
    ...
    <Radio value='7'>Bookmaker </Radio>
        ...
        <p
          ...
          >
            <MdLiveTv fontSize={"30px"} style={{ color: iconColor }} /> {/* Icon for Live Bet */}
            {t(`Live`)} {t(`Bet`)}{" "}
        ...
        <div className={` flex flex-wrap  items-center mt-8 gap-4 `}>
          ...
          <div className="bg-white flex items-center cursor-pointer shadow-md gap-2 rounded-md px-4 p-1">
            <p className="text-[16px] font-semibold text-gray-800">
              {t(`Total`)} {t(`Running`)} {t(`Bet`)} {t(`Amount`)}:
            </p>
        ...
        <div>
          ...
          >
            ...
            <div className="overflow-auto">
             {casinoBetData?.length>0&&<table className={`w-full`}>
                <thead>
                  <tr
                    style={{ backgroundColor: bg, color: "white" }}
                    className="text-left border border-gray-300 text-[12px]"
                  >
                    <th className="text-left min-w-[150px] px-3 p-2 border border-gray-300">
                      {t(`Username`)} / {t(`User`)} {t(`id`)}
                    </th>
                    <th className="min-w-[120px] px-3 p-2 border border-gray-300">
                      {t(`Game`)} {t(`id`)}{" "}
                    </th>
                    <th className="min-w-[120px] px-3 p-2 border border-gray-300">
                      {t(`Bet`)} {t(`Placed`)}
                    ...
                    <th className="min-w-[120px] px-3 p-2 border border-gray-300">{t(`Amount`)}</th>
                    <th className="min-w-[120px] px-3 p-2 border border-gray-300">
                      {t(`Win`)}/{t(`Loss`)}
                    </th>
                    <th className=" min-w-[120px] px-3 p-2 border border-gray-300">{t(`Status`)}</th>
                  </tr>
                </thead>
                <tbody>
                  {casinoBetData &&
                    casinoBetData?.map((item) => {
                      return (
                        ...
                        >
                          ...
                          <td className="border border-gray-300 px-3 p-2">{item?.Provider}</td>
                          <td className="border border-gray-300 px-3 p-2">
                          ...
                          <td
                            className={`font-bold border border-gray-300 px-3 p-2 ${
                              item.Status == "running"
                                ? "text-orange-400"
                                : item.WinLoss !== "0"
                                ? "text-green-600"
