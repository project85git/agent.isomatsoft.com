...
import { AiOutlineBell, AiOutlineHistory, AiOutlineShareAlt, AiOutlineTransaction } from 'react-icons/ai';
...
import { checkPermission, formatDate, formatDateTime } from '../../utils/utils';
import SingleLoginHistory from '../component/usermanageComponent/SingleLoginHistory';
import { FaLevelDownAlt, FaSignInAlt, FaUsers, FaUserShield } from 'react-icons/fa';
...
const SingleAdminManage = () => {
  ...
  const data = [
    ...
    { id: 2, title: "Deposits", icon: <RiLuggageDepositLine /> },
    { id: 3, title: "Withdrawals", icon: <BiMoneyWithdraw /> },
    { id: 4, title: "Transactions", icon: <AiOutlineTransaction /> },
    { id: 5, title: "Users", icon: <FaUsers /> },
    { id: 6, title: "Admins", icon: <FaUserShield /> },
    { id: 7, title: "Logins", icon: <FaSignInAlt /> },
    { id: 8, title: "Notifications", icon: <AiOutlineBell /> },
    { id: 9, title: "Level Manage", icon: <GiRank3 /> }
  ];
    ...
    return filterLevel[0]?.label||name
      ...
      let hasPermission = checkPermission(permissionDetails, "isOwnerAdmin");
      let check = hasPermission? true: false;
    ...
    <div className="flex flex-col md:flex-row justify-between mt-4 gap-4 w-[98%] m-auto  ">
        ...
        <div className="flex flex-col-reverse  md:flex-col gap-5">
          ...
          >
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-1 gap-2 mt-2">
              ...
              <div
                className={`flex justify-between cursor-pointer items-center gap-3 p-[6px] text-xs rounded-2xl`}
              >
                <p className='font-semibold'>{t(`2FA Authentication`)}</p>
                <Switch
                  name="is_2fa_enabled"
                  colorScheme="green"
                  isChecked={userData?.is_2fa_enabled == true ? true : false}
                  onChange={() => handleStatusChange("is_2fa_enabled")}
                />
              </div>
            </div>
          </div>
          ...
          <div
            ...
            className="  w-[100%] flex flex-col gap-4  bg-white   p-4 rounded-md "
          >
            ...
            <SendNotification userData={userData}/>
              ...
              {/* <button
                ...
                className={`w-[100%] ${bg} font-semibold text-white text-xs rounded-[5px] p-[7px]`}
              >
                Login User
              </button>
      ...
      <div className="w-[95%]  md:w-[90%] mx-auto">
        ...
        <div  style={{border:`1px solid ${border}60`}} className="w-[100%] flex flex-wrap lg:flex-nowrap bg-[white] rounded-md justify-between items-center p-1 md:p-3   ">
        <div className="flex w-[150px] lg:w-[100%] flex-col items-start gap-1">
            ...
            className={`px-2 text-xs py-[2px] mt-1 rounded-[5px] text-center text-white`}>{t(getLevelName(userData?.role_type))}</p>
          </div>
         
          <div className="flex w-[170px] lg:w-[100%] justify-center items-center flex-col gap-1">
        ...
        {active===5&&<AllUser userData={userData} />}
        {active===6&&<AllAdmin userData={userData} />}
        {active===7&&<SingleLoginHistory userData={userData}  role="alladmin"/> }
        {active===8&&<UserNotification userData={userData} role={userData?.role_type}/> }
        {active===9&&check&&<LevelManage adminId={userData?._id}/> }
        </div>
        </div>
  )
}
