...
const UserManage = () => {
  ...
  const getAllUser = async () => {
    ...
    }/api/admin/get-all-user-by-parent?page=${currentPage}&limit=${limit}`;
  ...
  <button
    ...
    className="flex-1 p-3 text-sm cursor-pointer font-bold text-white bg-green-500 rounded-md shadow-md hover:bg-green-600 transition-all text-center"
  >
    {usersCount.activeUserCount} {t("Active")}
  </button>
  <button
    onClick={() => setUserCategory("is_blocked")}
    className="flex-1 p-3 text-sm cursor-pointer font-bold text-white bg-red-500 rounded-md shadow-md hover:bg-red-600 transition-all text-center"
  >
    {usersCount.blockUserCount} {t("Blocked")}
  </button>
  {/* Uncomment and customize as needed
  <button
    onClick={() => setUserCategory("bet_active")}
    className="flex-1 p-3 text-sm cursor-pointer font-bold text-white bg-yellow-500 rounded-md shadow-md hover:bg-yellow-600 transition-all text-center"
  >
    {usersCount.betDeactiveUser} {t("Bet Suspend")}
  </button>
  */}
</div>
          ...
          <tr key={item._id} className="border-b border-gray-300">
            ...
            <td className="p-2 border-r border-gray-300">
              ...
              <Tooltip label={"Profile"} bg={bg} aria-label={`Profile`} hasArrow>
                ...
                />
              <BalanceAction userData={item} getData={getAllUser} title="Deposit" type="direct"/>
