...
const PlayerLoginHistory = () => {
  ...
  const getAllLoginHistory = async () => {
    ...
    }/api/login-history/get-history?page=${currentPage}&limit=${limit}&role=user`;
    if (search) {
      url += `&search=${search}`;
    }
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto rounded-lg p-2  bg-white border border-gray-200 ">
          <table className="min-w-full  ">
            <thead>
              <tr>
                <th className="py-2 px-4 text-nowrap text-left border-b">{t(`ID`)}</th>
                <th className="py-2 px-4 text-nowrap text-left border-b">
                  {t(`Username`)}
                </th>
                <th className="py-2 px-4 text-nowrap text-left border-b">{t(`Email`)}</th>
                <th className="py-2 px-4 text-nowrap text-left border-b">{t(`role`)}</th>
                ...
                <th className="py-2 px-4  text-nowrap text-center border-b">
                  {t(`Login`)} {t(`History`)}
                </th>
              </tr>
            </thead>
              ...
              {loginHistoryData.map((entry, index) => (
                <tr key={entry?._id} className="text-left">
                  ...
                  <td className="py-2 px-4 border-b">
                    <p>
                      {" "}
                      {entry?.created_time.split("T")[0]}{" "}
                      <span className="text-xs ml-1 font-semibold">
                        {" "}
                        ({entry?.created_time.split("T")[1]})
                      </span>
                    </p>
                  </td>
                  <td className="py-2 px-4 border-b">
                    <p>
                      {" "}
                      {entry?.updated_time.split("T")[0]}{" "}
                      <span className="text-xs ml-1 font-semibold">
                        {" "}
                        ({entry?.updated_time.split("T")[1]})
                      </span>
                    </p>
                  </td>
