...
const LoginHistory = () => {
  ...
  return (
    <div className=" w-[100%] md:p-4">
      ...
      <div className="flex flex-col lg:mt-10 md:flex-row items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <input
            ...
            className="mb-2 md:mb-0 md:mr-2 p-2 outline-none rounded-[8px]"
          />
          <select
            value={role}
            onChange={(e) => setRole(e.target.value)}
            style={{ border: `1px solid ${border}60` }}
            className="mb-2 w-[170px] md:mb-0 md:mr-2 p-2 outline-none rounded-[8px]"
          >
            <option value="alladmin">{t(`Select Role`)} </option>
            {user.user?.role_type && (
              <option value={"owneradmin"}>{t("owneradmin")}</option>
            )}
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto rounded-lg p-2  bg-white border border-gray-200 ">
          <table className="min-w-full  ">
            <thead>
              <tr>
                <th className="py-2 min-w-[60px]  text-nowrap text-left border-b">{t(`ID`)}</th>
                <th className="py-2 min-w-[140px]  text-nowrap text-left border-b">
                  {t(`Username`)}
                </th>
                <th className="py-2 min-w-[200px]  text-nowrap text-left border-b">{t(`Email`)}</th>
                <th className="py-2 min-w-[100px]  text-nowrap  text-left border-b">{t(`role`)}</th>
                ...
                <th className="py-2   text-nowrap text-left border-b">
                  {t(`Login`)} {t(`History`)}
                </th>
              </tr>
            </thead>
            <tbody>
              {loginHistoryData.map((entry, index) => (
                <tr key={entry?._id} className="text-left">
                  ...
                  <td className="py-2  border-b">{entry?.username}</td>
                  <td className="py-2  border-b">{entry?.email}</td>
                  <td className="py-2   border-b">{entry?.role}</td>

                  <td className="py-2  border-b">
                    
                    <p>
                      {" "}
                      {formatDateTime(entry?.created_time).split(" ")[0]}{" "}
                      <span className="text-xs ml-1 font-semibold">
                        {" "}
                        ({formatDateTime(entry?.created_time).split(" ")[1]})
                      </span>
                    </p>
                  </td>
                  <td className="py-2 border-b">
                    <p>
                      {" "}
                      {formatDateTime(entry?.updated_time).split(" ")[0]}
                      <span className="text-xs ml-1 font-semibold">
                        {" "}
                        ({formatDateTime(entry?.updated_time).split(" ")[1]})
                      </span>
                    </p>
                  </td>
